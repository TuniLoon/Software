"""
app.py
Flask web server with cloud integration and ML anomaly detection.
"""
...
def process_packet(packet_string: str):
    ...
    try:
        ...
        if anomaly_detector.is_initialized:
            ...
            # Only mark as anomaly if both is_anomaly and confidence > threshold (0.7)
        ...
        # Telegram status alerts (separate from anomaly alerts)
        try:
            if telegram_bot.enabled:
                telegram_bot.check_and_alert(data)
                telemetry_data['cloud_status']['telegram'] = 'enabled'
        except Exception as e:
            app.logger.error(f"Telegram check error: {e}")
...
if __name__ == '__main__':
    print("=" * 60)
    print("  TuniLoon Ground Station with ML Anomaly Detection")
